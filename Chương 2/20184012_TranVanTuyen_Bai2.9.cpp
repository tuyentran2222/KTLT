...
double sigmoid_fast(double x){
    ...
    int i = (x + LIMIT) / delta;
    return  y[i] + (x - (-LIMIT + i*delta)) * (y[i+1] - y[i]) / delta;
}
...
int main() {
    ...
    clock_t start =clock();
    res1=sigmoid_slow(xval);
	clock_t finish =clock();
	double slow=(double)(finish-start)/CLOCKS_PER_SEC;
	
	
	
    start =clock();
	res2=sigmoid_fast(xval);
	finish =clock();
	double fast=(double)(finish-start)/CLOCKS_PER_SEC;
	cout<<fast;
    if ((res1==res2) && (slow/fast > 1.3)) {
